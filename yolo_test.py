...
def draw_labels(boxes, confs, colors, class_ids, classes, img, width): 
	...
	for i in range(len(boxes)):
		if i in indexes:
			...
			#myobj = gTTS(text=currentLabel+" ahead", lang='en', slow=False)
...
def image_detect(img_path): 
	...
	draw_labels(boxes, confs, colors, class_ids, classes, image, width)
	while True:
		key = cv2.waitKey(1)
		if key == 27:
			break
...
def webcam_detect():
	...
	while True:
		...
		boxes, confs, class_ids = get_box_dimensions(outputs, height, width)
		draw_labels(boxes, confs, colors, class_ids, classes, frame, width)
		key = cv2.waitKey(1)
		if key == 27:
			break
	cap.release()
...
# python yolo_test.py --webcam=True
